# 1 "Peripherial_Slave.S"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 296 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Peripherial_Slave.S" 2
;***************************************************************************
;
;Filename: Blimp_Peripheral_Slave.S
;Date: 03/30/2026
;File Version: 1
...
# 27 "Peripherial_Slave.S" 2
# 1 "./pic16f1788.inc" 1
# 28 "Peripherial_Slave.S" 2
# 1 "./88SetUp.inc" 1
;-----------------------------------------------------------------------------------------------------------------------------------------------
...
;--------------------------------------------------------PORT A------------------------------------------------------------------------------
    ...
    BANKSEL PORTA
    MOVLW 0x00
    MOVWF PORTA ;Set PORTA LOW -Jason
...
;--------------------------------------------------------PORT B------------------------------------------------------------------------------
    ...
    BANKSEL PORTB
    MOVLW 0x00
    MOVWF PORTB ;Set PORTB low -Jason
...
;--------------------------------------------------------PORT C------------------------------------------------------------------------------
    ...
    MOVWF LATC
    BANKSEL TRISC
    MOVLW 0X18 ;SDA and SCL as inputs
    MOVWF TRISC
    BANKSEL WPUC
    MOVLW 0X00
    MOVWF WPUC
    ...
    BANKSEL PORTC
    MOVLW 0x00
    MOVWF PORTC ;Set PORTC LOW -Jason
...
;--------------------------------------------------------INTERRUPT CONTROL-----------------------------------------------------------
    BANKSEL INTCON
    MOVLW 0XC0 ;Global and peripheral interrupts Enabled
    MOVWF INTCON

;--------------------------------------------------------PERIPHERAL ONE-----------------------------------------------------------------
    BANKSEL PIE1
    MOVLW 0X0B ;Enable ((PIR1) and 07Fh), 3, TMR2, and TMR1 Interrupt -Jason
    MOVWF PIE1
...
;--------------------------------------------------------TIMER TWO-----------------------------------------------------------------------
    ...
    BANKSEL PR2
    MOVLW 0x64 ;move (0110|0100) to w
    MOVWF PR2 ;Set count so that TMR2 compares itself decimal 100, 0x64

    BANKSEL T2CON
    MOVLW 0x05 ;move (0000|0101) to w
    MOVWF T2CON ;Set Prescaler to 1:4, Postscaler to 1:1, Enable TMR2
...
; BSF PIE1,((PIE1) and 07Fh), 5 ; ENABLE RECIEVE INTERRUPT FOR UART
    RETURN
# 29 "Peripherial_Slave.S" 2
# 1 "./I2C_Handeler.inc" 1
;***************************************************************************
;Filename: I2C_Handeler
;Date: 03/30/2026
;File Version: 1
;Author: Jason Permann
;Co-Author:
...
;***************************************************************************************
    BANKSEL SSP1ADD
    MOVLW 0x20
    MOVWF SSP1ADD ;Peripheral Address Read=0x11; Write=0x10
    BANKSEL SSP1CON1
    MOVLW 0X36
    MOVWF SSP1CON1 ;Enable SDA and SCL Pins, Release Clock, and Set as 7 bit address Slave
    BANKSEL SSP1CON2
    MOVLW 0X09
    ...
    BANKSEL PORTB
    BSF PORTB,1 ;made it to I2C program
    RETURN
...
ADDRESS_RX:
 BANKSEL SSP1BUF
 MOVF SSP1BUF,0 ;Clear Address out of Buffer
 BANKSEL I2C_RX_COUNTER
...
JOY3_UD EQU 0X7A ;Register to Save Joystick 3 Up-Down Data
JOY3_LR EQU 0X7B ;Register to Save Joystick 3 Left-Right Data
BUTTONS_1 EQU 0X7C ;Register to Save BUTTONS Data Packet 1
BUTTONS_2 EQU 0X7D ;Register to Save BUTTONS_2 Data Packet 2
;----------------------Bank 0 Gerneral purpouse---------------------------------
PERIOD_VALUE EQU 0x20
PWM_CHECK_BYTE EQU 0x21
PW1_VALUE EQU 0x22 ;PWM of JOY3_UD
PW2_VALUE EQU 0x23 ;Right Servo Pitch
I2C_TIMEOUT_COUNT EQU 0x24 ;Track I2C timeout
...
;*** SFR SETUP **********
    ...
    BANKSEL JOY3_LR
    MOVLW 0x80 ;Decimal 128
    MOVWF JOY3_LR ;Begin With all Joysticks at "Center
    RETURN
...
PWM_STARTUP:
    ...
    BANKSEL PW2_VALUE
    MOVLW 0x0F ;decimal 128, start camera at middle
    MOVWF PW2_VALUE
...
PERIOD_END:
    ...
    BCF STATUS,0 ;clear carry bit (check for negative number)
    BANKSEL PWM_CHECK_BYTE
    MOVLW 0x00
    MOVWF PWM_CHECK_BYTE
...
WEIGHT_JOYSTICK_3_DATA:
    BANKSEL PW1_VALUE
    CLRF PW1_VALUE

    BANKSEL JOY3_UD
    MOVLW 0xFF
    XORWF JOY3_UD,1 ;We found that more thrust was achived with flipping the polarity of the motors witch means we need to invert the throttle.
    MOVF JOY3_UD,0

   ;BANKSEL JOY3_UD ;If you need to uninvert the throttle uncomment this code and comment out the above section
   ;MOVF JOY3_UD,0
...
CAMERA_PWM:
    BANKSEL BUTTONS_1
    BTFSC BUTTONS_1,0 ;Move camera down
    CALL CAMERA_DOWN

    BANKSEL BUTTONS_1
    BTFSC BUTTONS_1,3 ;Move camera Up
    CALL CAMERA_UP

    BANKSEL BUTTONS_1
    BTFSC BUTTONS_1,1 ;Move camera to middle
    CALL CAMERA_MIDDLE
